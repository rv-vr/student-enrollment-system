...
<section class="login-shell">
	<div class="login-panel">
		...
		{#if alertMessage}
			<div class="banner" data-tone={alertTone} role="alert" aria-live="assertive">{alertMessage}</div>
		{/if}
		...
		<form class="login-form" onsubmit={handleLogin}>
			...
			<div class="login-actions">
				...
				{#if session}
					<p class="helper">Already signed in as <strong>{session.user.name}</strong>.</p>
				{/if}
			</div>
		</form>
	</div>
