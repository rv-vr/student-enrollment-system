    ...
    <div class="flex flex-col md:flex-row justify-between gap-6">
      <div>
        ...
        <P class="lede mt-2 text-gray-600 dark:text-gray-400">
          Assign instructors, break courses into live offerings, and control
          each section's schedule and capacity independently.
        </P>
      </div>
  ...
  <div class="admin-grid space-y-6">
    <Card size="xl" class="shadow-none border-slate-200 max-w-none p-4">
      <div class="panel-heading mb-6 flex justify-between items-start">
        <div>
          <Heading
            ...
            >Schedule offering</Heading
          >
        </div>
        <Badge color="blue" class="rounded-full px-3 py-1 font-bold"
      ...
      <form class="section-form space-y-6" onsubmit={handleSubmit}>
        <div
          class="form-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6"
        >
          <div>
        ...
        >
          <div class="block-heading flex justify-between items-start mb-4">
            <div>
              <Heading
                ...
                >Schedule Array Builder</Heading
              >
              <P size="sm" class="text-gray-500 dark:text-gray-400"
                >Add day, time, and class type entries for this section.</P
              >
            </div>
            <Badge color="indigo" class="rounded-full px-3"
              >{scheduleItems.length} items</Badge
            >
          </div>
          ...
          {#if scheduleItems.length > 0}
            <div
              class="schedule-list mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"
            >
              {#each scheduleItems as item, index (item.day + item.time + item.type + index)}
                <Card
                  size="sm"
                  class="flex-row justify-between items-center gap-4 p-3 bg-white dark:bg-gray-800"
                >
                  <div class="min-w-0">
          ...
          {:else}
            <div
              class="empty-copy mt-4 p-4 bg-white/50 border border-dashed border-slate-200 rounded-lg text-center text-gray-500 dark:bg-gray-800/50 dark:border-gray-600"
            >
            ...
            {#each requests as req (req.id)}
              <TableBodyRow>
                ...
                <TableBodyCell class="text-gray-900 dark:text-white"
                  >{req.section?.sectionName}</TableBodyCell
                >
                <TableBodyCell class="text-gray-900 dark:text-white">
          ...
          <TableBody>
            {#each sections as section (section.id)}
              <TableBodyRow>
                ...
                <TableBodyCell class="text-gray-900 dark:text-white"
                  >{section.sectionName}</TableBodyCell
                >
                <TableBodyCell class="text-gray-900 dark:text-white"
                  >{section.instructorName}</TableBodyCell
                >
                <TableBodyCell class="text-gray-900 dark:text-white text-center"
                  >{section.capacity}</TableBodyCell
                >
                <TableBodyCell>
                ...
                <TableBodyCell>
                  <div
                    class="text-xs text-gray-500 dark:text-gray-400 leading-relaxed italic"
                  >
        ...
        >
          ...
          <TableBody>
            {#each roster as row (row.id)}
              <TableBodyRow>
                <TableBodyCell class="text-gray-900 dark:text-white"
                  >{row.student.id}</TableBodyCell
                >
                <TableBodyCell>
                  <div
                    class="font-bold text-gray-900 dark:text-white truncate max-w-[200px]"
                  >
                    {row.student.name}
                  </div>
                  <div class="text-xs text-gray-500 dark:text-gray-400">
                ...
                <TableBodyCell>
                  <Badge
                    color={row.status === "finalized" ? "green" : "blue"}
                ...
                <TableBodyCell
                  class="text-gray-500 dark:text-gray-400 italic truncate max-w-[200px]"
                >
                  {row.remark ?? "—"}
                </TableBodyCell>
              </TableBodyRow>
