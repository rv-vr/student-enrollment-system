<script lang="ts">
	...
	async function refreshStudentCourses(studentId: string) {
		...
		try {
			...
			studentCourses = null
			showAlert(error instanceof Error ? error.message : 'Unable to load your courses.', 'error')
		} finally {
			loading = false
		}
	}
...
<section class="page-grid">
	<div class="panel">
		<div class="panel-header">
			...
			<div class="meta">
				{studentCourses?.enrollments.length ?? 0} active enrollment{(studentCourses?.enrollments.length ?? 0) === 1 ? '' : 's'}
			</div>
		</div>
					...
					<table>
						...
						<tbody>
							{#each studentCourses.enrollments as enrollment (enrollment.id)}
								<tr>
									<td class="course-code">{enrollment.course?.code ?? enrollment.courseCode}</td>
									<td>
										<div class="course-title">{enrollment.course?.title ?? enrollment.courseCode}</div>
										{#if enrollment.course?.prerequisiteCodes?.length}
											<p class="meta">Prerequisite path completed</p>
										{/if}
									</td>
									<td>
										<span class="pill" data-tone={enrollment.grade == null ? 'warn' : 'good'}>
											{enrollment.grade == null ? 'No Grade Assigned' : String(enrollment.grade)}
										</span>
									</td>
									<td>
									...
									<td>
										...
										>
											{actionCourseCode === enrollment.courseCode ? 'Dropping…' : 'Drop Course'}
										</button>
									</td>
								</tr>
							{/each}
