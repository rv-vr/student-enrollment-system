...
export function getDbFromBindings(bindings: { DB: D1Database } | { env: { DB: D1Database } }) {
  const db: D1Database = "DB" in bindings ? bindings.DB : (bindings.env.DB as D1Database);
